...
def prerae_batch(size, batch):
    last = size % batch
    pre_batch = np.full((size,1), batch)
    if (last==0):
        return pre_batch
    else:
        pre_batch[size-1] = last
        return pre_batch
    

def train(train_d,dev_d,test_d,kfold,dataset, neighbor_sample_size, embed_dim, n_depth, l2_weight, lr, optimizer_type,
          batch_size, aggregator_type, n_epoch, embed_smile, callbacks_to_add=None, overwrite=True):
    config = ModelConfig()
    ...
    train_data=np.array(train_d)[:len(train_d)%batch_size*-1]
    valid_data=np.array(dev_d)[:len(dev_d)%batch_size*-1]
    test_data=np.array(test_d)[:len(test_d)%batch_size*-1]
    ...
    if not os.path.exists(model_save_path) or overwrite:
        ...
        model.fit(x_train=[train_data[:, :1], train_data[:, 1:2],x_train_drug_one_smile,x_train_drug_two_smile], y_train=train_data[:, 2:3],
                  x_valid=[valid_data[:, :1], valid_data[:, 1:2],x_valid_drug_one_smile,x_valid_drug_two_smile], y_valid=valid_data[:, 2:3])
    ...
    y_two = valid_data[:, 2:3]
    
    auc, acc, f1,aupr = model.score(x=x_one, y=y_two)
    ...
    model.load_best_model()
    auc, acc, f1, aupr = model.score(x=[test_data[:, :1], test_data[:, 1:2], 
