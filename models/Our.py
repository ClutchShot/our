...
# batch_shape=(self.config.batch_size,1
        input_smiles_one = Input(shape=(300, ), name='input_smiles_one', dtype='float32' )
        input_smiles_two = Input(shape=(300, ), name='input_smiles_two', dtype='float32')
        ...
        # drug1_squeeze_embed  = Lambda( lambda x: K.concatenate(x, axis=1))([drug1_squeeze_embed,input_smiles_one])
        # drug2_squeeze_embed  = Lambda( lambda x: K.concatenate(x, axis=1))([drug2_squeeze_embed,input_smiles_two])

        drug1_squeeze_embed = Lambda( lambda x: self.fusion(x[0], x[1]), name = "fusion1")([drug1_squeeze_embed,input_smiles_one])
        drug2_squeeze_embed = Lambda( lambda x: self.fusion(x[0], x[1]), name = "fusion2")([drug2_squeeze_embed, input_smiles_two])

        drug_drug_score = Lambda(lambda x: K.concatenate(x, axis=1), name = "concat_drugs")([drug1_squeeze_embed, drug2_squeeze_embed])
        ...
        layer1 = Sequential(
            [ Dense(self.config.batch_size,activation=None),
    ...
    # 'binary_crossentropy'
    # BinaryCrossentropy(from_logits=True, reduction=losses_utils.Reduction.NONE)

        score = Lambda(lambda x: K.sigmoid(x))(layer3)

        model = Model([input_drug_one, input_drug_two, input_smiles_one, input_smiles_two], score)
        model.compile(optimizer=self.config.optimizer,
                      loss=BinaryCrossentropy(), metrics=['acc'])
        return model

    def fusion(self, drug, smile):
        druglayer_structure = Dense(self.config.embed_dim,activation=None)(smile)
        druglayer_KG = Dense(self.config.embed_dim,activation=None)(drug)
        
        druglayer_structure_reshape = Lambda( lambda x: K.expand_dims(x, axis=-1))(druglayer_structure)
        druglayer_KG_respahe = Lambda( lambda x: K.expand_dims(x, axis=-1))(druglayer_KG)

        druglayer_structure_reshape_per = Lambda(lambda x: K.permute_dimensions(x, (0, 2, 1)))(druglayer_structure_reshape) 
        druglayer_KG_respahe_per = Lambda(lambda x: K.permute_dimensions(x, (0, 2, 1)))(druglayer_KG_respahe) 

        entity_matrix = Lambda( lambda x : multiply(x))([druglayer_structure_reshape, druglayer_KG_respahe_per])
        entity_matrix_reverse = Lambda( lambda x : multiply(x))([druglayer_KG_respahe, druglayer_structure_reshape_per])

        entity_global = Flatten()(entity_matrix)
        entity_global_reverse =  Flatten()(entity_matrix_reverse) 


        entity_matrix_reshape = Lambda( lambda x: K.expand_dims(x, axis=-1))(entity_matrix)
        entity_matrix_reverse_reshape = Lambda( lambda x: K.expand_dims(x, axis=-1))(entity_matrix_reverse)

        batch = []

        for b in range(int(self.config.batch_size / 128)):
            batch.append([i+(b*128) for i in range(128)])

        for i,b in enumerate(batch):

            entity_data = Lambda(lambda x: K.gather(x,b))(entity_matrix_reshape)

            conv1 = Sequential(
            [ Conv2D(filters=8, kernel_size=(5,5)),
            BatchNormalization(momentum=0.1),
            MaxPooling2D(pool_size=(2, 2)), 
            Activation('relu')])(entity_data)
            ...
            entity_matrix_reshape_new = Flatten()(conv1_2) 

            fc1 = Sequential(
            [Dense(self.config.embed_dim,activation=None), 
            BatchNormalization(momentum=0.1),
            Activation("relu")
            ])(entity_matrix_reshape_new)

            if i == 0:
                embedding_data = fc1
            else:
                embedding_data = Lambda( lambda x: K.concatenate(x,axis=0))([embedding_data,fc1])

        global_local_before = Lambda(lambda x: K.concatenate(x,axis=1))([embedding_data, entity_global])

        fc2_global = Sequential(
            [Dense(self.config.embed_dim,activation=None),
            Activation("relu")
        ])(global_local_before)
        ...
        for i,b in enumerate(batch):
            ...
            entity_data_2 = Lambda(lambda x: K.gather(x,b))(entity_matrix_reverse_reshape)

            conv2 = Sequential(
            [ Conv2D(filters=8, kernel_size=(5,5)),
            BatchNormalization(momentum=0.1),
            MaxPooling2D(pool_size=(2, 2)), 
            Activation('relu')])(entity_data_2)
            conv2_2 = Sequential(
            [ Conv2D(filters=8, kernel_size=(5, 5)),
            BatchNormalization(momentum=0.1),
            MaxPooling2D(pool_size=(2, 2)),
            ...
            entity_matrix_reverse_reshape_new = Flatten()(conv2_2) 

            fc1_2 = Sequential(
            [Dense(self.config.embed_dim,activation=None), 
            BatchNormalization(momentum=0.1),
            Activation("relu")
            ])(entity_matrix_reverse_reshape_new)

            if i == 0:
                embedding_data_2 = fc1_2
            else:
                embedding_data_2 = Lambda( lambda x: K.concatenate(x,axis=0))([embedding_data_2,fc1_2])

        global_local_before_reverse = Lambda(lambda x: K.concatenate(x, axis=1))([embedding_data_2, entity_global_reverse])

        fc2_global_reverse = Sequential(
            [Dense(self.config.embed_dim,activation=None),
            Activation("relu")
        ])(global_local_before_reverse)


        multi = Lambda( lambda x: multiply(x))([druglayer_structure, druglayer_KG])
        multi_drug = Sequential(
            [Dense(self.config.embed_dim,activation=None)]
        )(multi)

        out3 = Activation('relu')(multi_drug)

        out_concat = Lambda(lambda x: K.concatenate(x, axis=1))([drug, smile, fc2_global, fc2_global_reverse, out3])
        # out_concat = Concatenate(axis=1)([drug, smile, fc2_global, fc2_global_reverse, out3])
        return out_concat
    ...
    def add_metrics(self, x_train, y_train, x_valid, y_valid):
        self.callbacks.append(KGCNMetric(x_train, y_train, x_valid, y_valid,
                                         self.config.aggregator_type, self.config.dataset, self.config.K_Fold, self.config.batch_size))
